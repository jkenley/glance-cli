...
import { hasBinaryArtifacts, nuclearCleanText, sanitizeAIResponse } from "../core/text-cleaner";
    ...
    try {
      ...
      if (attempt < attempts) {
        ...
        if (onRetry) {
          ...
          logger.debug(`Retry attempt ${attempt}/${attempts} after ${waitTime}ms`);
        }
...
export function isPremiumModel(model: string): boolean {
  ...
  return premiumPrefixes.some(prefix => model.toLowerCase().startsWith(prefix));
}

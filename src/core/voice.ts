...
export class VoiceSynthesizer {
	...
	async synthesize(
		...
		if (this.client && this.apiKey) {
			try {
				...
				console.warn(
					chalk.yellow("⚠️  ElevenLabs failed, falling back to local TTS"),
				);
				console.error(error);
			}
		}
	...
	private async synthesizeWithElevenLabs(
		...
		try {
			...
			// Save to file
			const outputPath =
				options.outputFile || join(tmpdir(), `glance-audio-${Date.now()}.mp3`);
	...
	private getOptimalVoiceConfig(options: VoiceOptions): {
		...
		const langVoices =
			LANGUAGE_VOICES[language as keyof typeof LANGUAGE_VOICES] ||
			LANGUAGE_VOICES.en;
		...
		if (requestedVoice) {
			...
			voiceId =
				langVoices.default ||
				langVoices[Object.keys(langVoices)[0] as keyof typeof langVoices];
		}

		// Get optimal model for language
		const model =
			options.model ||
			LANGUAGE_MODELS[language as keyof typeof LANGUAGE_MODELS] ||
	 ...
	 */
	private getLanguageOptimizedSettings(
		language: string,
		options: VoiceOptions,
	) {
		const baseSettings = {
	...
	async listVoices(_language?: string): Promise<string[]> {
		const platform = process.platform;
		...
		if (this.client && this.apiKey) {
			try {
				...
				result.push(
					"  glance https://example.com --voice antoine -l fr --read",
				);
				result.push(
					"  glance https://example.com --voice isabella -l es --read",
				);
				result.push("");
				result.push(
					"⚠️  NOTE: Only the voices listed above work with simple names.",
				);
				result.push(
					"   For other voices, use the full voice ID (long string).",
				);
				result.push("");

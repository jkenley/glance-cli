...
function validateLanguage(
	lang: string,
): asserts lang is keyof typeof LANGUAGE_MAP {
...
const PROMPT_TEMPLATES = {
	...
	systemInstruction: (
		langName: string,
	) => `You are an expert content analyst and summarizer with deep expertise in extracting, analyzing, and presenting information clearly and accurately.
    ...
    **Language Requirements:**
	...
	standardSummary: (
		emoji: boolean,
	) => `**Task:** Create a comprehensive yet concise summary of the webpage content.
    ...
    **Quality Checks:**
	...
	tldr: (
		emoji: boolean,
	) => `**Task:** Distill the ENTIRE webpage into ONE powerful sentence (maximum 25 words).
    ...
    **Example of Great TL;DR:**
	...
	keyPoints: (
		emoji: boolean,
	) => `**Task:** Extract 6-10 key points that comprehensively represent the webpage's content.
    ...
    **Coverage Checklist:**
	...
	eli5: (
		emoji: boolean,
	) => `**Task:** Explain this webpage's content as if teaching a curious 10-year-old child.
    ...
    **Example Transformation:**
	...
	customQuestion: (
		question: string,
		emoji: boolean,
	) => `**Task:** Answer the following question using ONLY information found in the provided webpage text.
    ...
    **Forbidden:**
	...
	translation: (
		langName: string,
	) => `**Task:** Translate the entire provided content to ${langName} while preserving ALL original formatting and structure.
    ...
    **Output Format:**
	 ...
	 */
	smartFormatting:
		() => `**Task:** Intelligently format the provided raw text content to enhance readability while preserving ALL original meaning and context.
...
async function openaiSummarize(
	...
	const defaultMaxTokens = options.tldr
		? 100
		: options.keyPoints
			? 800
			: options.eli5
	...
	if (hasBinaryArtifacts(rawContent)) {
		console.error(
			"🚨 CRITICAL: Binary artifacts in OpenAI response! Applying emergency cleaning...",
		);
...
async function geminiSummarize(
	...
	if (hasBinaryArtifacts(rawText)) {
		console.error(
			"🚨 CRITICAL: Binary artifacts in Gemini response! Applying emergency cleaning...",
		);
 ...
 */
async function _withRetry<T>(fn: () => Promise<T>, retries = 3): Promise<T> {
	...
	for (let i = 0; i < retries; i++) {
		try {
			...
			console.warn(
				chalk.yellow(`Retry ${i + 1}/${retries} after ${delay}ms...`),
			);

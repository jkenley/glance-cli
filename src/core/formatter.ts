...
function sanitizeMetadata(metadata: any): PageMetadata {
    ...
    return {
        ...
        keywords: Array.isArray(metadata.keywords)
            ? metadata.keywords.slice(0, 10).map((k: any) => sanitizeText(String(k), 50))
            : undefined,
 ...
 */
function formatTerminal(summary: string, options: Required<FormatOptions>): string {
    const parts: string[] = [];
    ...
    // Source URL
    parts.push(
        chalk.bold.cyan(`${EMOJI.source} Source: `) +
        chalk.underline(options.url)
    );
    ...
    if (meta.title) {
        parts.push(
            chalk.bold.green(`${EMOJI.title} Title: `) +
    ...
    if (!options.compact) {
        ...
        if (options.processingTime !== undefined) {
            processingParts.push(`${EMOJI.time} ${(options.processingTime / 1000).toFixed(1)}s`);
        }
    ...
    if (options.includeTimestamp && !options.compact) {
        parts.push("");
        parts.push(chalk.dim(`${EMOJI.date} Generated: ${new Date().toLocaleString()}`));
    }
 ...
 * Format as Markdown
 */
function formatMarkdown(summary: string, options: Required<FormatOptions>): string {
    const parts: string[] = [];
    ...
    if (options.model || options.processingTime !== undefined) {
        ...
        if (options.processingTime !== undefined) {
            processingParts.push(`Time: ${(options.processingTime / 1000).toFixed(1)}s`);
        }
        if (options.fromCache) {
...
function formatHTML(summary: string, options: Required<FormatOptions>): string {
    const meta = options.metadata;
    const title = options.customQuestion ? "Answer" : (options.customTitle || "Summary");
    ...
    parts.push("  <style>");
    parts.push("    body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 800px; margin: 40px auto; padding: 20px; line-height: 1.6; color: #333; }");
    parts.push("    h1 { color: #2c3e50; border-bottom: 3px solid #3498db; padding-bottom: 10px; }");
    parts.push("    .metadata { background: #f8f9fa; padding: 15px; border-radius: 5px; margin: 20px 0; }");
    parts.push("    .metadata p { margin: 5px 0; color: #666; }");
    parts.push("    .content { margin: 30px 0; }");
    parts.push("    .question { background: #fff3cd; padding: 15px; border-left: 4px solid #ffc107; margin: 20px 0; }");
    parts.push("    .footer { margin-top: 40px; padding-top: 20px; border-top: 1px solid #ddd; font-size: 0.9em; color: #666; }");
    ...
    if (options.customQuestion) {
        ...
        parts.push(`    <strong>Question:</strong> ${escapeHTML(options.customQuestion)}`);
        parts.push("  </div>");
    }

    // Metadata
    parts.push('  <div class="metadata">');
    parts.push(`    <p><strong>Source:</strong> <a href="${escapeHTML(options.url)}" target="_blank">${escapeHTML(options.url)}</a></p>`);
    if (meta.title) {
        parts.push(`    <p><strong>Title:</strong> ${escapeHTML(meta.title)}</p>`);
    }
    if (options.includeMetadata) {
        if (meta.author) {
            parts.push(`    <p><strong>Author:</strong> ${escapeHTML(meta.author)}</p>`);
        }
        if (meta.publishDate) {
            parts.push(`    <p><strong>Published:</strong> ${escapeHTML(meta.publishDate)}</p>`);
        }
    }
    if (options.model) {
        parts.push(`    <p><strong>Model:</strong> ${escapeHTML(options.model)}</p>`);
    }
    parts.push("  </div>");
 ...
 */
function formatPlainText(summary: string, options: Required<FormatOptions>): string {
    const parts: string[] = [];

    // Title
    const title = options.customQuestion ? "ANSWER" : (options.customTitle || "SUMMARY").toUpperCase();
    parts.push(title);
 ...
 */
export function formatOutputWithMetadata(summary: string, options: FormatOptions): FormattedOutput {
    const content = formatOutput(summary, options);

    return {
        content,
        format: options.format || (options.json ? "json" : options.markdown ? "markdown" : "terminal"),
 ...
 */
export function autoDetectFormat(filePath: string): "markdown" | "json" | "html" | "plain" {

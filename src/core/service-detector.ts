 ...
 */
async function checkOllama(endpoint: string = 'http://localhost:11434'): Promise<ServiceStatus> {
  try {
...
export async function detectServices(
  ...
  const availableAI = aiServices.filter(s => s.available);
  ...
  const availableVoice = voiceServices.filter(s => s.available);
  ...
  if (verbose) {
    ...
    aiServices.forEach(service => {
      ...
      const badge = service.type === 'free' ? chalk.green('[FREE]') : chalk.yellow('[PAID]');
      console.log(`  ${icon} ${service.name} ${badge} ${service.reason ? chalk.dim(`- ${service.reason}`) : ''}`);
    });
    ...
    console.log(chalk.bold('\n🎯 Selected:'));
    console.log(`  AI: ${preferredAI !== 'none' ? chalk.green(preferredAI) : chalk.red('No AI service available')}`);
    console.log(`  Voice: ${preferredVoice !== 'none' ? chalk.green(preferredVoice) : chalk.red('No voice service available')}`);
    
    if (preferFree && (openai.available || gemini.available || elevenlabs.available)) {
...
export async function getDefaultModel(ollamaEndpoint?: string): Promise<string> {
  ...
  switch (detection.ai.preferred) {
    case 'ollama':
      ...
      try {
        const res = await fetch(`${ollamaEndpoint || 'http://localhost:11434'}/api/tags`);
        if (res.ok) {
          const data = await res.json();
          const models = (data as any).models || [];
          if (models.length > 0) {
            // Prefer llama3 if available, otherwise use the first model
            const llama3 = models.find((m: any) => m.name.includes('llama3'));
...
export function showCostWarning(service: string, model?: string): void {
  const warnings: Record<string, string> = {
    ...
    elevenlabs: 'Using ElevenLabs voices - This will consume API credits! Use --read without API key for free local TTS.',
  };
